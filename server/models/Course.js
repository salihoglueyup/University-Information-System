...
CourseSchema.post('save', async function(doc) {
    ...
    } catch(err) { console.error('Meili Index Error (Course Save):', err.message) }
});
...
CourseSchema.post('findOneAndUpdate', async function(doc) {
    if (doc) {
        try {
            const obj = { id: doc._id.toString(), code: doc.code, title: doc.name, instructor: doc.instructor, credits: doc.credit };
            await meiliClient.index('courses').addDocuments([obj]);
        } catch(err) { console.error('Meili Index Error (Course Update):', err.message) }
    }
});
...
CourseSchema.post('findOneAndDelete', async function(doc) {
    if (doc) {
        ...
        catch(err) { console.error('Meili Index Error (Course Delete):', err.message) }
    }
});
